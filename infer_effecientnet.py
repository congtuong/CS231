...
import torchvision
from torchvision.models import vgg16_bn, efficientnet_v2_l
...
class YoloEff(nn.Module):
    def __init__(self,anchors, num_classes=2, eff = efficientnet_v2_l(pretrained=True), device = "cuda"):
        super(YoloEff, self).__init__()
        self.num_anchors = 5
        self.num_classes = num_classes
        self.anchors = anchors
        self.eff_layers = nn.Sequential(*list(eff.features.children()))
        self.device = device
        # fit through the first 23 layers of vgg then add extra layers with 3 convs and a maxpool
        self.extra = nn.Sequential(
            CNNBlock(320, 320, kernel_size=3, padding=1, stride=1),
            CNNBlock(320,320, kernel_size=3, padding=1, stride=1),
            CNNBlock(320, 320, kernel_size=3, padding=1, stride=1),
        ...
        self.skip_module = nn.Sequential(
            CNNBlock(320, 40, kernel_size=1, padding=0, stride=1),
        )

        self.final = nn.Sequential(
            CNNBlock(320 + 160, 1024, kernel_size=3, padding=1),
    ...
    def forward(self, x):
        ...
        x = x.to(self.device)
        print(x.shape)
        x = self.eff_layers(x).view(-1, 320, 26,26)
        # print(x.shape)

        skip_x = self.skip_module(x).to(self.device)

        skip_x = skip_x.view(-1, 40, o_size, 2, o_size, 2).contiguous()
        skip_x = skip_x.permute(0, 3, 5, 1, 2, 4).contiguous()
        skip_x = skip_x.view(-1, 40*4, o_size, o_size)
...
def Yolo_loss(pred_targets, target_tensor,anchors, num_classes=2):
    ...
    # print(pred_x1.shape, gt_x1.shape)
...
device = 'cuda'
PATH = "/mlcv1/WorkingSpace/Personal/tuongbck/project_231/weights_eff_new/"

# train = load_dataset(train_path)
# test = load_dataset(test_path)
# val = load_dataset(val_path)

# model = YoloEff(anchors,num_classes=2, eff = efficientnet_v2_l(pretrained=True), device=device).to(device)
# model.load_state_dict(torch.load(PATH + "model_yololoss_cuda_40.pt"))
# train_fn(model, train, val,PATH, epochs=41,collate_fn=collate_fn, batch_size=8)

weights = "/mlcv1/WorkingSpace/Personal/tuongbck/project_231/weights_eff_new_lower/model_yololoss_cuda_40.pt"

# # save

model = YoloEff(anchors,num_classes=2, eff = efficientnet_v2_l(pretrained=True), device='cpu').to('cpu')
    ...
    with torch.no_grad():
        for i, (path,imgs, _) in tqdm(enumerate(test_loader)):
            ...
            for idx in range(len(pred_bbox)):
                ...
                img, thresholded_list = visualize_prediction(np.array(pred_bbox[idx]),pred_cls[idx].cpu().view((845,2)),np.array(pred_conf[idx].cpu().reshape((845,1))), imgs[idx],threshold)
                output_list.append({path[idx]:thresholded_list})
                video_list.append(img)
                # img.save(save_path + f"/images/img_{i*batch_size+idx}.jpg")
    video = cv2.VideoWriter(save_path + "output_eff_lower_non_nms.avi", cv2.VideoWriter_fourcc(*'DIVX'), 1, (416,416))
    for img in video_list:
        video.write(img)
    video.release()
    with open(save_path + "predictions_eff_lower_non_nms.json", "w") as f:
...
visualize_prediction_to_video(test, model,anchors,'cpu', "/mlcv1/WorkingSpace/Personal/tuongbck/project_231/test_new_output/", batch_size = 8, collate_fn = collate_fn,threshold = 0.5)
